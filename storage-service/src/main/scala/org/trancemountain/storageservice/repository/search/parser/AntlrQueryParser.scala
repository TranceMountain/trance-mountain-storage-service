...
@Lazy
@Service
class AntlrQueryParser extends IQueryParser {

	import AntlrQueryParser.LOG

	override def parse(query: String): IQuery = {
		val parser = new TMQLVisitorImpl
		parser.parse(query)
	}
}
