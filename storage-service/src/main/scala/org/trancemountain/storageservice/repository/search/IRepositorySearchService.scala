...
/**
	...
	*/
trait IRepositorySearchService {
	def nodeDocumentCount: Long
	def indexNodes(nodes: Seq[INode], asynchronous: Boolean = false)
	def nodeIDs(query: SolrQuery, revision: RepositoryRevisionNumber): Seq[String]
	def truncateToMinimumRevision(revision: RepositoryRevisionNumber)
